...
export class HistorialVentas implements OnInit, OnDestroy {
  ...
  totalBoletas  = computed(() => this.ventasSemana().filter((c) => c.tipo_comprobante === '03').length);
  totalFacturas = computed(() => this.ventasSemana().filter((c) => c.tipo_comprobante === '01').length);
  ...
  kpiTotalVentas   = computed(() => this.kpiSede()?.total_ventas    ?? this.totalVentas());
  kpiNumeroVentas  = computed(() => this.kpiSede()?.cantidad_ventas ?? this.numeroVentas());
  kpiTotalBoletas  = computed(() => this.kpiSede()?.total_boletas   ?? this.totalBoletas());
  kpiTotalFacturas = computed(() => this.kpiSede()?.total_facturas  ?? this.totalFacturas());
