...
export class GenerarVentasAdministracion implements OnInit, AfterViewInit {
  ...
  tipoDocBusqueda = signal<number | null>(null); // tipo doc para buscar
  ...
  onDocumentoNuevoClienteChange(valor: string): void {
    ...
    // Detectar por descripción (más robusto que buscar solo "DNI" o "RUC")
    ...
    this.ventasService.consultarDocumentoIdentidad(valor).subscribe({
      next: (res) => {
        ...
        if (res?.nombreCompleto) {
          ...
          // Si es RUC y trae dirección, pre-llenar también
          if (esRuc && res.direccion) {
            this.nuevoClienteForm.address = res.direccion;
          }
          this.messageService.add({
            severity: 'success',
  ...
  aplicarPromocion(promo: PromocionAdmin): void {
    ...
    this.messageService.add({
      ...
      detail: `${promo.concepto} — descuento: ${
        this.esPorcentaje(promo.tipo) ? `${promo.valor}%` : `S/. ${promo.valor.toFixed(2)}`
      }`,
      life: 3000,
    });
  }
