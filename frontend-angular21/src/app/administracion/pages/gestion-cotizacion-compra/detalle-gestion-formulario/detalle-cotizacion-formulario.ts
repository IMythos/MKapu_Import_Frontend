...
import { Quote, QuoteCliente, QuoteSede, QuoteProveedor } from '../../../interfaces/quote.interface';
...
export class DetalleCotizacionComponent implements OnInit {
  ...
  proveedor  = computed<QuoteProveedor | null>(() => this.cotizacion()?.proveedor ?? null);
  ...
  ngOnInit() {
    ...
    if (!id) { this.router.navigate(['/admin/cotizaciones-compra']); return; }
  ...
  enviar() {
    const id = this.cotizacion()?.id_cotizacion;
    if (!id) return;
    this.messageService.add({ severity: 'info', summary: 'Enviando...', detail: 'Generando y enviando cotización por email.' });
    this.quoteService.sendByEmail(id).subscribe({
      next:  (res) => this.messageService.add({ severity: 'success', summary: 'Email enviado', detail: `Cotización enviada a ${res.sentTo}` }),
  ...
  volver()   { this.router.navigate(['/admin/cotizaciones-compra']); }
  irEditar() { this.router.navigate(['/admin/editar-cotizacion', this.cotizacion()?.id_cotizacion]); }
}